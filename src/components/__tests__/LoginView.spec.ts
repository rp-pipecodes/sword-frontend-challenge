...
describe("LoginView.vue Test", () => {
  it("renders the component with all the elements", () => {
    ...
    expect(wrapper.find("h1").text()).toBe(i18n.global.t("auth.sign_in"));
    ...
    expect(submitButton.text()).toBe(i18n.global.t("auth.sign_in"));

    const signupLink = wrapper.findComponent({ name: "RouterLinkStub" });
    expect(signupLink.text()).toBe(i18n.global.t("auth.click_to_signup"));
  ...
  it("validates the error messages", async () => {
    ...
    expect(authStore.logIn).toHaveBeenCalledTimes(0);
